...
function EventSlider() {
	...
	var time = document.getElementById("animationSpeed").value * 1000;
	updateTimer=setInterval(function(){myTimer();},time);
...
// function called to adjust the interval between time requests of the player
...
function d3CreateEventSlider() {	
	...
	height = 20; //185 - margin.top - margin.bottom; //300 - margin.top - margin.bottom;
		
	//Adding the plot framwork to the application
	var svg = addSliderPlot(margin, width, height +10);
...
function changeEventStep() {
	...
	var selectedItem = circles["_groups"][0][selectedEventSliderIndex];
