...
require([
    ...
    return declare("WMSLayerWithTime", esri.layers.DynamicMapServiceLayer, {
	    ...
	    constructor: function(url, parameterToLoad) {
		  ...
		  wmsDimensionsLoadedEvent = new CustomEvent('Event', { detail: { parameterToLoad: parameterToLoad }});
...
function wmsLayerGetDimensionValues(dimName,layerName)
{
	...
	var layerDimensionInfo = wmsLayerDimensionValues[layerName].dimensions;
	//console.log(wmsLayerDimensionValues[layerName]);
	//console.log(wmsLayerDimensionValues[layerName].dimensions);
	//console.log(layerDimensionInfo);
	//console.log(layerDimensionInfo[dimName]);
	//console.log(dimName);
	//console.log(layerDimensionInfo[dimName].values);
  if (!layerDimensionInfo[dimName]) {
    console.log(dimName + " is not defined in layerDimensionInfo");
    return [];
  }
...
function getWMSDimesionDefinition(wmsURL)
{
	...
	var dataUrl = wmsURL +  getCapUrlEnd;
...
//      if (getCapStore[wmsURL] !== undefined ){
//        var data = getCapStore[wmsURL];
//      }
//      else{
...
//      } 
	...
	if(proxy){esri.request(xhrArgs);}
	else{getCapabilities(dataUrl);}
}
	...
	for(childrenIndex = 0; childrenIndex < layerChildren.length; childrenIndex++)
	{
		...
		if(tagName.toLowerCase() == 'name')
		{
			...
			try {
				...
				console.log("no queryIndex... skipping");
				continue;
			}
			...
			for(index = 0; index < dimensionsDom.length; index++)
			{
				...
				try{
					...
					layerDimensionInfo.dimensions[wmsDimenProp].defaultValue = values[0];
					console.log("No default value setting to first value");
				}
